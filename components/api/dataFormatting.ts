...
import type {
  ...
  Day,
  Daily,
} from "../../types/api/formatGenericalData.types";
import type {
  CurrentWeather,
  HourWeather,
  DayWeather,
...
export function formatCurrentWeather(data: Current): CurrentWeather {
  return {
    ...
    rain: ifRainy(
      data.weather[0].main,
      data.rain == undefined ? 0 : data.rain["1h"]
    ),
    snow: ifSnowed(
      data.weather[0].main,
      data.snow == undefined ? 0 : data.snow["1h"]
    ),
  };
}

export function formatHourWeather(data: Hourly) {
  let weatherData,
    weatherHourly = [];
...
export function formatDayWeather(data: Daily) {
  let weatherData,
    weatherDaily = [];
...
function formatCurrentDayWeather(data: Day): DayWeather {
  return {
    ...
    snow: ifSnowed(
      data.weather[0].main,
      data.snow == undefined ? 0 : data.snow
    ),
  };
}
